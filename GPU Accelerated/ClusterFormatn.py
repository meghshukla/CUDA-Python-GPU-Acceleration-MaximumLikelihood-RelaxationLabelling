...
def ClusterFormation(Img,ClusterDict):
    iD=[]
    ID=1
    ...
    for k in range(np.shape(Img)[-1]-1):
        Band+=[[]]
    Class=[]
    for classValue in ClusterDict.keys():
        for segment in ClusterDict[classValue]:
            for i in range(segment[0],segment[2]+1):
                for j in range(segment[1],segment[3]+1):
                    iD.append(ID)
                    ID+=1
                    for k in range(np.shape(Img)[-1]):
                        Band[k].append(Img[i,j][k])
                    Class.append(classValue)
    #Creating Dict
